...
class Day02(BaseDay):
    games: dict[int, list[defaultdict[str, int]]] = {}
    ...
    def part2(self) -> None:
        ...
        def maxpower(grabs: list[defaultdict[str, int]]) -> int:
            return (
                max(t["red"] for t in grabs)
                * max(t["green"] for t in grabs)
                * max(t["blue"] for t in grabs)
            )
